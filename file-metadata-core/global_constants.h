...
template <typename T>
static constexpr bool anyChar = std::is_same<T, char>::value || std::is_same<T, uchar>::value;

template <typename T>
static constexpr bool anyInt = std::is_same<T, qint16>::value || std::is_same<T, qint32>::value || std::is_same<T, qint64>::value ||
...
static constexpr int v1Len = 128;
static constexpr int headerLen = 10;
