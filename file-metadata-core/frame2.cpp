...
bool Frame2::parse() {
    qDebug() << "Frame2: starting to parse" << Qt::endl;
    parseHeader();
    return parseData();
}

bool Frame2::parseHeader() {
    qDebug() << "Frame2: starting to parse header" << Qt::endl;
...
QString Frame2::getEncodingDependentString(ulong len) const {
    qDebug() << "Frame2: getting encoding dependent for V 2 string with length" << len << Qt::endl;
    return FileHolder::getEncodingDependentString(two, len);
}

QList<QString> Frame2::getList(ulong len, QChar separator) const {
    qDebug() << "Frame2: getting list for V 2 with length" << len << "and deliminator" << separator << Qt::endl;
