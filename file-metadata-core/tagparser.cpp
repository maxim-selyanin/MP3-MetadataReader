...
TagParser::TagParser(TagVersion version, Binary &file) {
    switch (version) {
    case two:{
        qDebug() << "TagParser: set v2" << Qt::endl;
        tag = new(nothrow) Binary::V22(file);
        return;
    }
    case three:{
        qDebug() << "TagParser: set v3" << Qt::endl;
        tag = new(nothrow) Binary::V23(file);
        return;
    }
    case four:{
        qDebug() << "TagParser: set v4" << Qt::endl;
        tag = new(nothrow) Binary::V24(file);
        return;
    }
    case noTag:{
        qDebug() << "TagParser: got noTag, nothing is set" << Qt::endl;
        return;
    }
    }
}
...
ParsingResult TagParser::parse() const {
    if (tag) {
        qDebug() << "TagParser: starting parsing" << Qt::endl;
        bool fine = tag->parse();
        qDebug() << "TagParser: ended parsing" << Qt::endl;
        if (fine)
            return success;
        else
            return fail;
